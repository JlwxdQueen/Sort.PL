...
function bubbleSort(arr: number[]): number[] {
    ...
    for (let i = 0; i < n - 1; i++) {
        let swapped: boolean = false; // Flag to track if a swap occurred during this pass
        ...
        if (!swapped) {
            break; // Exit the loop early to improve efficiency
        }
    }
...
const sortedArray: number[] = bubbleSort(arrayToSort);
